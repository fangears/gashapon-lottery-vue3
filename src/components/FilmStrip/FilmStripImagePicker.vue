...
const selectedIds = computed(() => store.config.filmImageIds ?? []);
...
const confirmPicker = () => {
  store.setFilmImageIds(tempSelected.value);
  pickerVisible.value = false;
};

const removeFromSelection = (id: string) => {
  store.removeFilmImageId(id);
};

const clearSelection = () => {
  store.clearFilmImageIds();
};
...
<template>
  <section class="card admin-film">
    ...
    <div v-else class="film-empty">
      ...
      <div class="film-empty-subtitle">请先在“图片管理”上传素材，然后在此选择用于胶片展示的图片</div>
    </div>
